...
export const registerStep1Schema = z
  .object({
    fullName: z
      ...
      .min(
        3,
        "Nome completo é obrigatório e deve ter pelo menos 3 caracteres."
      ),
...
export const registerStep2ClienteSchema = z.object({
  celular: z
    ...
    .regex(
      /^\(\d{2}\)\s\d{5}-\d{4}$/,
      "Formato de celular inválido: (XX) XXXXX-XXXX"
    )
  ...
  cpf: z
    ...
    .regex(
      /^\d{3}\.\d{3}\.\d{3}-\d{2}$/,
      "Formato de CPF inválido: XXX.XXX.XXX-XX"
    )
...
export const registerStep2GuiaSchema = z.object({
  ...
  numCadastro: z
    ...
    .regex(
      /^\d{2}\.\d{6}\.\d{2}-\d{1}$/,
      "Formato de Cadastur inválido: XX.XXXXXX.XX-X"
    )
...
export const createRegisterStep2Schema = (userType: "CLIENTE" | "GUIA") => {
  return userType === "CLIENTE"
    ? registerStep2ClienteSchema
    : registerStep2GuiaSchema;
};
...
export const completeGoogleRegistrationSchema = z.discriminatedUnion(
  "userType",
  [clienteGoogleSchema, guiaGoogleSchema]
);

export type CompleteGoogleRegistrationData = z.infer<
