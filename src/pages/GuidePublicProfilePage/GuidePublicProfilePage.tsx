...
const mockGuide = {
  ...
  activities: [
    ...
    { id: 3, title: "Poço Azul e Poço Encantado", image: "https://via.placeholder.com/300x200?text=Poço+Azul", price: 200.00 },
  ],
  reviews: [
    { id: 1, touristName: "Mariana Silva", rating: 5, comment: "Carlos é um guia incrível! Super atencioso, conhece tudo sobre o local e nos passou muita segurança. A viagem foi perfeita!", date: "10 de Julho, 2025" },
    { id: 2, touristName: "João V.", rating: 5, comment: "Experiência única! A organização foi impecável e o conhecimento do Carlos sobre a fauna e flora enriqueceu demais o passeio. Recomendo de olhos fechados.", date: "28 de Junho, 2025" },
    { id: 3, touristName: "Beatriz Costa", rating: 4, comment: "A travessia foi desafiadora e maravilhosa. O guia foi ótimo, só achei que o ritmo poderia ser um pouco mais leve no primeiro dia.", date: "15 de Maio, 2025" },
  ]
};
    ...
    return (
        <div className="flex items-center">
            {[...Array(totalStars)].map((_, index) => (
                <Star key={index} size={18} className={index < Math.round(rating) ? 'star-filled' : 'star-empty'} />
            ))}
        </div>
    );
};
    ...
    <div className="guide-profile-container flex min-h-screen flex-col items-center">
      ...
      <main className="w-full max-w-5xl mx-auto mt-10 md:mt-20 px-4">
        ...
        <section className="stats-container grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12">
            <div className="stat-item"><Star size={24} /><div><strong>{mockGuide.stats.averageRating.toFixed(1)}</strong><p>Avaliação Média</p></div></div>
            <div className="stat-item"><ThumbsUp size={24} /><div><strong>{mockGuide.stats.totalActivities}</strong><p>Atividades Realizadas</p></div></div>
            <div className="stat-item"><Award size={24} /><div><strong>{mockGuide.stats.yearsOfExperience}</strong><p>Anos de Experiência</p></div></div>
        </section>

        {/* --- Catálogo de Atividades --- */}
        <section className="mb-12">
          <h2 className="section-title">Atividades oferecidas por {mockGuide.name.split(' ')[0]}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
            {mockGuide.activities.map(activity => (
              <Link to={`/atividade/${activity.id}`} key={activity.id} className="activity-card-link bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:-translate-y-1">
                ...
                <div className="p-4">
                  ...
                  <p className="text-md font-semibold activity-price mt-2">R$ {activity.price.toFixed(2)}</p>
                </div>
              </Link>
            ))}
          </div>
