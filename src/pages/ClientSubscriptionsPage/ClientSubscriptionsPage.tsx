			...
			>
				...
				<main className="flex-1 flex justify-center items-center pt-20">
					<p className="text-lg text-red-600">
						Acesso negado. Apenas clientes podem acessar esta página.
					</p>
				</main>
				<Footer />
			</div>
		...
		>
			...
			<main className="flex-1 w-full max-w-6xl mx-auto px-4 pt-24 pb-12">
				<div className="mb-8">
					...
					<p className="text-gray-600">Acompanhe suas inscrições em passeios</p>
				</div>
				...
				{loading ? (
					...
					<div className="bg-white rounded-lg shadow-sm p-8 text-center border border-[rgba(137,143,41,0.1)]">
						...
						<p className="text-gray-500 mb-4">Você ainda não se inscreveu em nenhum passeio.</p>
						<Link
							to="/explorar"
								...
								<div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
									<div className="flex-1">
										<div className="flex items-start justify-between mb-3">
											...
											<div
												className={`px-3 py-2 rounded-full border text-sm font-medium flex items-center gap-2 whitespace-nowrap ${statusColors[subscription.status as keyof typeof statusColors] || "bg-gray-100 text-gray-800 border-gray-200"}`}
											>
										...
										<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-gray-600">
											<div className="flex items-center gap-2">
												<Calendar size={16} />
												<span>
													{new Date(subscription.horarioDisponivel.data_hora).toLocaleDateString(
														"pt-BR",
													)}
												</span>
											</div>
											<div className="flex items-center gap-2">
												<Clock size={16} />
												<span>
													{new Date(subscription.horarioDisponivel.data_hora).toLocaleTimeString(
														"pt-BR",
														{
															hour: "2-digit",
															minute: "2-digit",
											...
											<div className="mt-3 p-3 bg-gray-50 rounded-lg">
												<div className="flex justify-between items-center text-sm">
													...
													>
														R${" "}
														{(
															parseFloat(subscription.passeio.valor) *
															subscription.quantidade_pessoas
														)
															.toFixed(2)
															.replace(".", ",")}
													</span>
