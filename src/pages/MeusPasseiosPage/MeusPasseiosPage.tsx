...
const mockPasseios = [
	{
		id: "1",
		nome: "Travessia do Vale do Pati",
		data: "25/08/2025",
		participantes: 8,
		capacidade: 10,
		preco: 1250.0,
	...
	{
		id: "2",
		nome: "Cachoeira da Fumaça por Cima",
		data: "30/08/2025",
		participantes: 5,
		capacidade: 12,
		preco: 150.0,
	...
	{
		id: "3",
		nome: "Poço Azul e Poço Encantado",
		data: "10/09/2025",
		participantes: 12,
		capacidade: 12,
		preco: 200.0,
...
const TabButton = ({ label, activeTab, setActiveTab }: any) => (
	<button
		...
		style={{
			color: activeTab === label.toLowerCase() ? "var(--verde-oliva)" : "",
			borderColor: activeTab === label.toLowerCase() ? "var(--verde-oliva)" : "",
		}}
	>
		{label}
	</button>
				...
				<aside className="w-1/3 md:w-1/4 bg-white border-r border-gray-200 flex flex-col">
					<div className="p-4 border-b flex justify-between items-center">
						...
						<Link to="/criar-passeio" title="Criar Novo Passeio">
							<PlusCircle
								size={22}
								className="text-[var(--verde-vibrante)] hover:opacity-80 transition-opacity"
							/>
						</Link>
					</div>
					...
					<div className="p-4 md:p-6 space-y-6">
						{activeTab === "visão geral" && (
							<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
								<div className="bg-white p-4 rounded-lg shadow-sm border">
									<h4 className="text-sm text-gray-500">Participantes</h4>
									<p className="text-2xl font-bold">
										{selectedPasseio.participantes} / {selectedPasseio.capacidade}
									</p>
								</div>
								<div className="bg-white p-4 rounded-lg shadow-sm border">
									<h4 className="text-sm text-gray-500">Receita Estimada</h4>
									<p className="text-2xl font-bold">R$ {receitaEstimada.toFixed(2)}</p>
								</div>
								<div className="bg-white p-4 rounded-lg shadow-sm border">
									<h4 className="text-sm text-gray-500">Próxima Data</h4>
									<p className="text-2xl font-bold">{selectedPasseio.data}</p>
								</div>
							</div>
						)}
						{activeTab === "participantes" && (
							<div className="bg-white p-4 rounded-lg shadow-sm border">
								<h3 className="text-lg font-bold mb-4" style={{ color: "var(--verde-oliva)" }}>
								...
								<ul className="space-y-3">
									{mockParticipantes.map((p) => (
										...
										>
											...
											<span
												className={`px-2 py-1 text-xs font-semibold rounded-full ${p.status === "Confirmado" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
											>
												{p.status}
											</span>
										</li>
							...
							<div className="bg-white rounded-lg shadow-sm border h-[60vh] flex flex-col">
								<div className="flex-1 p-4 space-y-4 overflow-y-auto">
									...
									<div className="flex gap-2">
										<img src="https://i.pravatar.cc/40?u=a" className="w-8 h-8 rounded-full" />
										<div className="bg-gray-100 p-3 rounded-lg max-w-lg">
											<strong>Ana Clara:</strong> Pessoal, o ponto de encontro é na entrada do
											parque mesmo?
										</div>
									</div>
								</div>
								<div className="p-4 border-t flex items-center gap-2">
									<input
										...
										className="flex-1 px-4 py-2 border rounded-full focus:outline-none focus:ring-2 focus:ring-verde-vibrante"
									/>
									<button
						...
						{activeTab === "editar passeio" && (
							<div className="bg-white p-6 rounded-lg shadow-sm border">
								...
								<p className="text-gray-600">
									O formulário de edição do passeio apareceria aqui, permitindo alterar título,
						...
						{activeTab === "finanças" && (
							<div className="bg-white p-4 rounded-lg shadow-sm border">
								...
								<p className="text-gray-600">
									Uma tabela com o resumo de pagamentos dos participantes seria exibida aqui.
								</p>
							</div>
						)}
								...
								<div className="flex items-center justify-between p-4 border-t border-red-200">
									<div>
										...
										<p className="text-sm text-gray-600">
											Isso irá notificar todos os participantes e iniciar o processo de reembolso.
										</p>
									</div>
									<button className="px-4 py-2 text-sm border border-red-500 text-red-600 rounded-lg hover:bg-red-50">
										Cancelar Passeio
									</button>
								</div>
							</div>
