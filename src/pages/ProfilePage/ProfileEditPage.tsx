...
export default function ProfileEditPage() {
	...
	const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (name === "celular") {
			...
			setFormData((prev) => ({
				...prev,
				[name]: name === "idade" ? parseInt(value, 10) || 0 : value,
			}));
		}
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const dataToSend: UpdatePerfilData = {
				...formData,
				idade: Number(formData.idade),
			};
			await PerfilService.updateMyProfile(dataToSend, newProfilePicture || undefined);
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						<div className="flex flex-col sm:flex-row items-center gap-6">
							<div className="relative">
								<img
									src={
										preview ||
										`https://ui-avatars.com/api/?name=${formData.nome}&background=898f29&color=fff`
									}
									alt="Foto de Perfil"
								...
								<button
									...
									className="absolute bottom-0 right-0 bg-gray-700 text-white p-1.5 rounded-full hover:bg-gray-800 transition-colors"
								>
									<Camera size={16} />
								</button>
								<input
						...
						<div className="flex justify-end gap-4 pt-6 border-t">
							...
							<button
								type="submit"
								disabled={loading}
								className="px-6 py-2 text-white rounded-lg disabled:opacity-50 flex items-center gap-2 font-medium bg-green-600 hover:bg-green-700"
							>
								{loading ? (
									"Salvando..."
								) : (
