...
import { AlertCircle, ArrowLeft, Calendar, Clock, DollarSign, Edit3, Image as ImageIcon, Plus, Save, Trash2, Users, X } from "lucide-react";
    ...
    <div className="flex flex-col min-h-screen" style={{ backgroundColor: "var(--background-claro)" }}>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 pt-24 pb-12">
        ...
        <div className="bg-white rounded-lg shadow-md border border-[rgba(137,143,41,0.1)] mb-6">
          <div className="p-4 md:p-6 border-b border-gray-200 flex flex-col md:flex-row items-start md:items-center gap-4">
            {passeio.imagens && passeio.imagens.length > 0 ? (
              <img src={passeio.imagens[0].url_imagem} alt={passeio.titulo} className="w-full md:w-32 h-32 object-cover rounded-lg border" />
            ) : (
            ...
            <div className="flex-1">
              ...
              <h1 className="text-2xl font-bold" style={{ color: "var(--verde-oliva)" }}>{passeio.titulo}</h1>
              <div className="flex flex-wrap gap-x-4 gap-y-2 mt-2 text-sm text-gray-600">
                <span className="flex items-center gap-1.5"><Users size={14} /> {passeio.qtd_pessoas || "N/A"} vagas no total</span>
                <span className="flex items-center gap-1.5"><Clock size={14} /> {passeio.duracao_passeio || "N/A"} min</span>
                <span className="flex items-center gap-1.5"><DollarSign size={14} /> R$ {passeio.valor || "N/A"}</span>
              </div>
            </div>
            <button onClick={handleNewHorario} className="inline-flex items-center gap-2 px-4 py-2 text-white rounded-lg transition-colors hover:opacity-90 shadow-sm w-full md:w-auto justify-center" style={{ backgroundColor: "var(--verde-vibrante)" }}>
        ...
        {showForm && (
            <div className="bg-white rounded-lg shadow-md border border-[rgba(137,143,41,0.1)] mb-6 animate-fade-in-down">
                <div className="p-6 border-b border-gray-200 flex items-center justify-between">
                    <h2 className="text-lg font-semibold" style={{ color: "var(--verde-oliva)" }}>{editingHorario ? "Editar Horário" : "Adicionar Novo Horário"}</h2>
                    <button onClick={() => { setShowForm(false); setEditingHorario(null); }} className="p-2 text-gray-400 hover:text-gray-600 rounded-full"><X size={20} /></button>
                </div>
                <div className="p-6">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <input type="date" name="data" value={formData.data} onChange={handleInputChange} className={`w-full px-4 py-3 border rounded-lg transition-colors ${ errors.data_hora ? "border-red-500" : "border-gray-300 focus:border-verde-vibrante focus:ring-2 focus:ring-green-500/20" }`} />
                            </div>
                            <div>
                              <label className="block text-sm font-medium text-gray-700 mb-2">Hora *</label>
                              <input type="time" name="hora" value={formData.hora} onChange={handleInputChange} className={`w-full px-4 py-3 border rounded-lg transition-colors ${ errors.data_hora ? "border-red-500" : "border-gray-300 focus:border-verde-vibrante focus:ring-2 focus:ring-green-500/20" }`} />
                            </div>
                            {errors.data_hora && <p className="text-red-500 text-sm mt-1 flex items-center gap-1 col-span-2"><AlertCircle size={14} />{errors.data_hora}</p>}
                          </div>
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">Máximo de vagas para o passeio: {passeio.qtd_pessoas || "N/A"}</p>
                          </div>
                        </div>
                        <div className="flex justify-end gap-4 pt-4 border-t border-gray-100">
                          ...
                          <button type="submit" disabled={loading} className="px-6 py-2 text-white rounded-lg disabled:opacity-50 flex items-center gap-2 font-medium" style={{ backgroundColor: "var(--verde-vibrante)" }}>
                            {loading ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Salvando...</>) : (<><Save size={18} />{editingHorario ? "Atualizar Horário" : "Criar Horário"}</>)}                          </button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-md border border-[rgba(137,143,41,0.1)]">
          <div className="p-6 border-b border-gray-200"><h2 className="text-lg font-semibold" style={{ color: "var(--verde-oliva)" }}>Horários Disponíveis ({horarios.length})</h2></div>
          <div className="p-4 md:p-6">
            {horarios.length === 0 ? (
              <div className="text-center py-12">
                <Calendar size={48} className="mx-auto text-gray-400 mb-4" />
                <h3 className="text-lg font-medium text-gray-600">Nenhum horário criado</h3>
                <p className="text-gray-500 mb-4 text-sm">Crie o primeiro horário para este passeio clicando no botão acima.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {horarios.map((horario) => (
                  <div key={horario.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-center gap-4 mb-3 sm:mb-0">
                        <div className="w-10 h-10 rounded-full flex items-center justify-center bg-gray-100 flex-shrink-0"><Calendar size={18} className="text-gray-500" /></div>
                        <div>
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-center">
                      <button onClick={() => handleEdit(horario)} className="p-2 text-gray-500 hover:text-blue-600 rounded-full hover:bg-blue-50" title="Editar"><Edit3 size={16} /></button>
                      <button onClick={() => handleDelete(horario.id)} className="p-2 text-gray-500 hover:text-red-600 rounded-full hover:bg-red-50" title="Excluir"><Trash2 size={16} /></button>
                    </div>
                  </div>
                ))}
              </div>
