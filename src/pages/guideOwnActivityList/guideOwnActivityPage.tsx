    ...
    <div className="flex flex-col min-h-screen" style={{ backgroundColor: "var(--background-claro)" }}>
      ...
      <main className="flex-1 flex flex-col items-center px-2 py-8 md:px-0 pt-24">
        <div className="w-full max-w-4xl">
          <h1 className="text-2xl font-bold mb-6" style={{ color: "var(--verde-oliva)" }}>Meus Passeios</h1>
          {loading || isLoading ? (
            <div className="flex justify-center items-center py-20 text-gray-600">Carregando...</div>
          ) : passeios.length === 0 ? (
                ...
                <div key={passeio.id} className="bg-white rounded-lg shadow-md border p-6">
                  <div className="flex gap-4 items-center mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="text-xl font-semibold mb-1" style={{ color: "var(--verde-oliva)" }}>{passeio.titulo}</h2>
                      <p className="text-gray-600 mb-1 line-clamp-2">{passeio.descricao}</p>
                      <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                        <span className="flex items-center gap-1"><Clock size={14} /> {passeio.duracao_passeio} min</span>
                        <span className="flex items-center gap-1"><Users size={14} /> {passeio.qtd_pessoas} pessoas</span>
                        <span className="flex items-center gap-1">Nível {passeio.nivel_dificuldade}/10</span>
                        <span className="flex items-center gap-1">R$ {parseFloat(passeio.valor).toFixed(2).replace('.', ',')}</span>
                      </div>
                    </div>
                  </div>
                  <div className="mt-2">
                      ...
                      <ul className="ml-2 mt-1 space-y-1">
                        {passeio.horariosDisponiveis.map((h) => (
                          <li key={h.id} className="text-gray-600 text-sm">
                            {new Date(h.data_hora).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })} - {h.vagas_disponiveis} vaga(s)
                          </li>
                        ))}
                      </ul>
                    )}
