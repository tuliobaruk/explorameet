...
export default function GuideSubscriptionsPage() {
	...
	useEffect(() => {
		const currentNotificationCount = notifications.filter(
			(n) => n.tipo === "INSCRICAO_CLIENTE",
		).length;
			...
			<div className="flex flex-col min-h-screen bg-gray-50">
				...
				<main className="flex-1 flex justify-center items-center pt-20">
					<p className="text-lg text-red-600">
						Acesso negado. Apenas guias podem acessar esta página.
					</p>
				</main>
				<Footer />
			</div>
			...
			<main className="flex-1 w-full max-w-7xl mx-auto px-4 pt-24 pb-12">
				...
				{loading ? (
					<div className="text-center py-12">
						<p className="text-lg text-gray-600">Carregando inscrições...</p>
					</div>
					...
					<div className="space-y-6">
						{subscriptions.map((sub) => {
							const currentStatus = statusConfig[sub.status as keyof typeof statusConfig] || {
								label: sub.status,
							...
							return (
								<div
									key={sub.id}
									className="bg-white rounded-lg shadow-md border transition-shadow hover:shadow-lg overflow-hidden"
								>
										...
										<div className="flex-1 p-6">
												...
												<div>
													...
													<div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-gray-600">
														<span className="flex items-center gap-2">
															<Users size={16} /> {sub.cliente.perfil.nome}
														</span>
														<span className="flex items-center gap-2">
															<Calendar size={16} />{" "}
															{new Date(sub.horarioDisponivel.data_hora).toLocaleDateString(
																"pt-BR",
															)}
														</span>
														<span className="flex items-center gap-2">
															<Clock size={16} />{" "}
															{new Date(sub.horarioDisponivel.data_hora).toLocaleTimeString(
																"pt-BR",
																{ hour: "2-digit", minute: "2-digit" },
															)}
														</span>
														<span className="flex items-center gap-2">
															<Users size={16} /> {sub.quantidade_pessoas} pessoa(s)
														</span>
													</div>
												</div>
												<div
													className={`px-3 py-1.5 rounded-full text-sm font-semibold flex items-center gap-2 ${currentStatus.color}`}
												>
													{currentStatus.icon}
													{currentStatus.label}
												</div>
											...
											<div className="border-t pt-4 mt-4 flex flex-wrap items-center gap-3">
												{sub.status === "pendente" && (
													<>
														<button
															onClick={() => updateSubscriptionStatus(sub.id, "confirmada")}
														...
														<button
															onClick={() => updateSubscriptionStatus(sub.id, "cancelada_guia")}
															disabled={updating === sub.id}
												...
												{sub.status === "confirmada" && (
													<>
														<button
															onClick={() => updateSubscriptionStatus(sub.id, "concluida")}
															disabled={updating === sub.id}
														...
														<button
															onClick={() => updateSubscriptionStatus(sub.id, "nao_compareceu")}
															disabled={updating === sub.id}
															className="btn-action bg-orange-500 hover:bg-orange-600"
														>
															<AlertCircle size={16} /> Não Compareceu
														</button>
													</>
												)}
												<button
													onClick={() => handleContactClick(sub.cliente.perfil)}
													className="btn-action bg-gray-600 hover:bg-gray-700"
												>
						...
						<div className="space-y-4">
							<div className="flex items-center gap-3">
								...
								<p className="text-gray-700">
									<strong className="font-semibold">Nome:</strong> {selectedClient.nome}
								</p>
							</div>
							<div className="flex items-center gap-3">
								<Phone size={20} className="text-gray-500" />
								<p className="text-gray-700">
									<strong className="font-semibold">Celular:</strong> {selectedClient.celular}
								</p>
							</div>
						</div>
						<div className="mt-6 flex flex-col sm:flex-row-reverse gap-3">
							<a
								href={`https://wa.me/${selectedClient.celular.replace(/\D/g, "")}`}
								target="_blank"
								rel="noopener noreferrer"
								className="w-full sm:w-auto px-5 py-2.5 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors flex items-center justify-center gap-2 font-semibold"
							>
								<MessageCircle size={18} /> WhatsApp
							</a>
							<a
								href={`tel:${selectedClient.celular.replace(/\D/g, "")}`}
								className="w-full sm:w-auto px-5 py-2.5 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors flex items-center justify-center gap-2 font-semibold"
							>
								<Phone size={18} /> Ligar
							</a>
						</div>
