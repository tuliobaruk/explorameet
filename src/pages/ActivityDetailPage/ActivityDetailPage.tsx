    ...
    return (
      <div className="flex flex-col min-h-screen" style={{ backgroundColor: "var(--background-claro)" }}>
        ...
        <main className="flex-1 flex justify-center items-center pt-20">
          <p className="text-lg" style={{ color: 'var(--verde-oliva)' }}>Carregando passeio...</p>
        </main>
        <Footer />
      </div>
    );
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-2/3">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              ...
              <div className="flex items-center gap-2 bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm font-medium"><Users size={16} />{passeio.qtd_pessoas || 0} vagas</div>
            </div>
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm mt-8 border border-[rgba(137,143,41,0.1)] flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-xl font-bold" style={{ color: 'var(--verde-oliva)' }}>{passeio.guia.perfil.nome}</h3>
                <Link to={`/guia/${passeio.guia.id}`} className="text-sm font-semibold hover:underline" style={{ color: 'var(--marrom-dourado)' }}>Ver perfil do guia</Link>
              </div>
            </div>
          </div>
          ...
          {passeio.avaliacoes && passeio.avaliacoes.length > 0 ? (
            <div className="space-y-6">
              {passeio.avaliacoes.map((avaliacao) => (
                <div key={avaliacao.id} className="bg-white p-5 rounded-lg shadow-sm" style={{ borderLeft: '4px solid var(--verde-vibrante)' }}>
            ...
            <div className="bg-white p-8 rounded-lg shadow-sm border text-center">
              <p className="text-gray-600">Este passeio ainda não tem avaliações. Seja o primeiro a avaliar!</p>
            </div>
          )}
        </div>
      </main>
