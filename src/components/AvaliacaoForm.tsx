...
export default function AvaliacaoForm({ onSubmit, loading }: AvaliacaoFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-white p-6 rounded-lg shadow-md border border-[rgba(137,143,41,0.1)]">
      <h3 className="text-xl font-bold" style={{ color: 'var(--verde-oliva)' }}>Deixe sua avaliação</h3>
      <div>
        ...
        <div className="flex items-center gap-1">
          {[1, 2, 3, 4, 5].map((star) => (
            <Star
              ...
              style={{ 
                color: star <= (hoverRating || currentRating) ? 'var(--amarelo-estrela, #facc15)' : '#d1d5db' 
              }}
              fill={star <= (hoverRating || currentRating) ? 'var(--amarelo-estrela, #facc15)' : 'transparent'}
      ...
      <div>
        ...
        <textarea
          ...
          {...register("comentario")}
        ></textarea>
        {errors.comentario && <p className="text-red-500 text-sm mt-1">{errors.comentario.message}</p>}
      </div>
      <button
        type="submit"
        disabled={loading}
