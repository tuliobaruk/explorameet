...
export default function AvaliacaoList({ avaliacoes }: AvaliacaoListProps) {
    ...
    useEffect(() => {
        const fetchClientes = async () => {
            ...
            const clienteIds = [...new Set(avaliacoes.map(a => a.cliente?.id).filter(Boolean) as string[])];
                ...
                return (
                    ...
                    >
                        <div className="flex items-start gap-4">
                            <img 
                                src={fotoAvaliador || `https://ui-avatars.com/api/?name=${nomeAvaliador.replace(' ', '+')}&background=898f29&color=fff`} 
                                alt={nomeAvaliador} 
